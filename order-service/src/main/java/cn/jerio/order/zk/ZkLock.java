...
@Slf4j
@Component
public class ZkLock implements InitializingBean {
     ...
     * 获取分布式锁
     */
    public boolean acquireDistributedLock(String path,String requireId) {
        ...
        while (true) {
            try {
                curatorFramework
                        ...
                        .forPath(keyPath,requireId.getBytes());
     ...
     * 释放分布式锁
     */
    public boolean releaseDistributedLock(String path,String requireId) {
        try {
            String keyPath = "/" + ROOT_PATH_LOCK + "/" + path;
            if (curatorFramework.checkExists().forPath(keyPath) != null) {
                byte[] bytes = curatorFramework.getData().forPath(keyPath);
                if (requireId.equals(new String(bytes))){
